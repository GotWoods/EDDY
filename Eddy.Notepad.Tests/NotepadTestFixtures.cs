...
/// <summary>
/// Shared building blocks for hand-built X12 fixtures used by the DocumentLoader tests. Mirrors
/// Eddy.Tests.x12/x12TestFixtures.cs (not referenced here: it lives in a different test project) so the
/// two suites exercise recognisably similar documents.
/// </summary>
internal static class NotepadTestFixtures
{
    /// <summary>A body of 6 segments that passes validation untouched (SE count is therefore 8).</summary>
    ...
    public static string SingleValidInterchange(
        string isaControl = "000000001", string groupControl = "2100", string stControl = "0001", string newline = "~\n")
    {
        var sb = new StringBuilder();
