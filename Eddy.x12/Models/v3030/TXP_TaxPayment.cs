...
public class TXP_TaxPayment : EdiX12Segment
{
	...
	[Position(08)]
	public string AmountQualifierCode3 { get; set; }
	...
	public override ValidationResult Validate()
	{
		...
		validator.IfOneIsFilled_AllAreRequired(x=>x.AmountQualifierCode2, x=>x.TaxAmount2);
		validator.IfOneIsFilled_AllAreRequired(x=>x.AmountQualifierCode3, x=>x.TaxAmount3);
		...
		validator.Length(x => x.AmountQualifierCode2, 1, 2);
		validator.Length(x => x.TaxAmount2, 1, 10);
		validator.Length(x => x.AmountQualifierCode3, 1, 2);
