...
public class G76Tests
{
	...
	{
		string x12Line = "G76*5*Ht*8*Sd*6*iu*8*t*U*W";

		var expected = new G76_TotalPurchaseOrder()
		{
			QuantityOrdered = 5,
			UnitOrBasisForMeasurementCode = "Ht",
			Weight = 8,
			UnitOrBasisForMeasurementCode2 = "Sd",
			Volume = 6,
			UnitOrBasisForMeasurementCode3 = "iu",
			OrderSizingFactor = 8,
			Amount = "t",
	...
	public void Validation_RequiredQuantityOrdered(decimal quantityOrdered, bool isValidExpected)
	{
		...
		subject.UnitOrBasisForMeasurementCode = "Ht";
	...
	[Theory]
	[InlineData(0, "", true)]
	[InlineData(8, "Sd", true)]
	[InlineData(8, "", false)]
	[InlineData(0, "Sd", false)]
	...
	[Theory]
	[InlineData(0, "", true)]
	[InlineData(6, "iu", true)]
	[InlineData(6, "", false)]
	[InlineData(0, "iu", false)]
