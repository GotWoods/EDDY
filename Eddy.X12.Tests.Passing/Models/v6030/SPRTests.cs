...
public class SPRTests
{
	...
	{
		string x12Line = "SPR*SX*6*7*5*c*q*5*N*7z";

		var expected = new SPR_SupplierRating()
		{
			RatingCategoryCode = "SX",
			MeasurementValue = 6,
			RangeMinimum = 7,
			RangeMaximum = 5,
			RatingSummaryValueCode = "c",
			Description = "q",
			MeasurementValue2 = 5,
			InformationStatusCode = "N",
			UnitOfTimePeriodOrIntervalCode = "7z",
		};
