...
public class G31Tests
{
	...
	{
		string x12Line = "G31*1*5O*6*G0*5*bF*2*l*D";
		...
		var expected = new G31_TotalInvoiceQuantity()
		{
			...
			Weight = 6,
			UnitOrBasisForMeasurementCode2 = "G0",
			Volume = 5,
			UnitOrBasisForMeasurementCode3 = "bF",
			EquivalentWeight = 2,
	...
	[Theory]
	[InlineData(0, "", true)]
	[InlineData(6, "G0", true)]
	[InlineData(6, "", false)]
	[InlineData(0, "G0", false)]
	...
	[Theory]
	[InlineData(0, "", true)]
	[InlineData(2, "G0", true)]
	[InlineData(2, "", false)]
	[InlineData(0, "G0", true)]
