...
public class LLX {
	...
	[SectionPosition(6)] public LS_LoopHeader? LoopHeader { get; set; }
	[SectionPosition(7)] public List<LLX_LOP> LOP {get;set;} = new();
	[SectionPosition(8)] public LE_LoopTrailer? LoopTrailer { get; set; }
	[SectionPosition(9)] public List<LLX_LSES> LSES {get;set;} = new();
	public ValidationResult Validate()
	{
		...
		validator.CollectionSize(x => x.LSP, 0, 10);
		validator.CollectionSize(x => x.LOP, 0, 30);
		...
		foreach (var item in LOP) validator.Results.AddRange(item.Validate().Errors);
