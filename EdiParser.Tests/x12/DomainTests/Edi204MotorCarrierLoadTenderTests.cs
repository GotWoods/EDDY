...
public class Edi204MotorCarrierLoadTenderTests
{
    ...
    public void ToDocument(string path, char seperator)
    {
        foreach (var file in Directory.GetFiles(path))
        {
            ...
            {
                newDocument.Sections.Add(documentSection);
            }
            

            var actual = newDocument.ToString(MapOptionsForTesting.x12DefaultEndsWithTilde);
            actual = actual.Replace("~", "~\r\n");//pretty formatting

            var expectedLines = data.Replace("\r\n", "\n").Split(seperator);
    ...
    public void ToDocumentSection()
    {
        ...
        sourceModel.CarrierStandardCarrierAlphaCode = "XXXX";
        sourceModel.ShipmentIdentificationNumber = "32523432";
        sourceModel.ShipmentMethodOfPaymentCode = "PP";
        ...
        sourceModel.ReferenceNumbers.Add(new KeyValuePair<string, string>("OTH", "1st type"));
        sourceModel.ReferenceNumbers.Add(new KeyValuePair<string, string>("ZZZ", "2nd type"));
        sourceModel.Notes.Add(new Note() { ReferenceCode = "ABC", Description = "Note1"});
        sourceModel.Notes.Add(new Note() { ReferenceCode = "123", Description = "Note2" });
        ...
        expected.Segments.Add(new B2_BeginningSegmentForShipmentInformationTransaction() { StandardCarrierAlphaCode = sourceModel.CarrierStandardCarrierAlphaCode, ShipmentIdentificationNumber = sourceModel.ShipmentIdentificationNumber, ShipmentMethodOfPaymentCode = sourceModel.ShipmentMethodOfPaymentCode});
        expected.Segments.Add(new B2A_SetPurpose() { TransactionSetPurposeCode = sourceModel.Purpose});
        expected.Segments.Add(new L11_BusinessInstructionsAndReferenceNumber() { ReferenceIdentification = sourceModel.ReferenceNumbers[0].Value, ReferenceIdentificationQualifier = sourceModel.ReferenceNumbers[0].Key });
        expected.Segments.Add(new L11_BusinessInstructionsAndReferenceNumber() { ReferenceIdentification = sourceModel.ReferenceNumbers[1].Value, ReferenceIdentificationQualifier = sourceModel.ReferenceNumbers[1].Key });
        expected.Segments.Add(new NTE_Note() { Description = sourceModel.Notes[0].Description, NoteReferenceCode = sourceModel.Notes[0].ReferenceCode });
        expected.Segments.Add(new NTE_Note() { Description = sourceModel.Notes[1].Description, NoteReferenceCode = sourceModel.Notes[1].ReferenceCode });
        ...
        for (int i = 0; i < Math.Max(actual.Segments.Count, expected.Segments.Count); i++) 
        {
    ...
    public void LoadDocument()
    {
        ...
        expected.CarrierStandardCarrierAlphaCode = "XXXX";
        expected.ShipmentIdentificationNumber = "9999955559";
        expected.ShipmentMethodOfPaymentCode = "PP";
        expected.Purpose = "04";
