using System.Collections.Generic;
using Eddy.Core.Attributes;
using Eddy.Core.Validation;
using Eddy.x12.Models.v3020;
using Eddy.x12.DomainModels.Transportation.v3020._217;

namespace Eddy.x12.DomainModels.Transportation.v3020;

public class Edi217_MotorCarrierLoadingAndRouteGuide {
	[SectionPosition(1)] public ST_TransactionSetHeader TransactionSetHeader { get; set; } = new();
	[SectionPosition(2)] public BLR_BeginningSegmentLoadingAndRouteGuide BeginningSegmentLoadingAndRouteGuide { get; set; } = new();
	[SectionPosition(3)] public List<LN1> LN1 {get;set;} = new();

	//Details
	[SectionPosition(4)] public LS_LoopHeader? LoopHeader { get; set; }
	[SectionPosition(5)] public List<LN1> DetailLN1 {get;set;} = new();
	[SectionPosition(6)] public LE_LoopTrailer? LoopTrailer { get; set; }
	[SectionPosition(7)] public List<LLX> LLX {get;set;} = new();

	//Summary
	[SectionPosition(8)] public SE_TransactionSetTrailer TransactionSetTrailer { get; set; } = new();


	public ValidationResult Validate()
	{
		var validator = new TransactionValidator<Edi217_MotorCarrierLoadingAndRouteGuide>(this);
		validator.Required(x => x.TransactionSetHeader);
		validator.Required(x => x.BeginningSegmentLoadingAndRouteGuide);
		

		validator.CollectionSize(x => x.LN1, 0, 999999);
		foreach (var item in LN1) validator.Results.AddRange(item.Validate().Errors);
		

		validator.CollectionSize(x => x.LN1, 0, 999999);
		validator.CollectionSize(x => x.LLX, 0, 999999);
		foreach (var item in DetailLN1) validator.Results.AddRange(item.Validate().Errors);
		foreach (var item in LLX) validator.Results.AddRange(item.Validate().Errors);
		validator.Required(x => x.TransactionSetTrailer);
		return validator.Results;
	}
}
