...
using Eddy.x12.DomainModels.Transportation.v3020._490;
...
public class Edi490_RateGroupDefinition {
	...
	[SectionPosition(7)] public List<LGY> LGY {get;set;} = new();
	[SectionPosition(8)] public List<LPR> LPR {get;set;} = new();
	[SectionPosition(9)] public List<LPT> LPT {get;set;} = new();
	[SectionPosition(10)] public List<LCD> LCD {get;set;} = new();
	...
	public ValidationResult Validate()
	{
		...
		validator.CollectionSize(x => x.LGY, 0, 500);
		validator.CollectionSize(x => x.LPR, 0, 500);
		validator.CollectionSize(x => x.LPT, 0, 500);
		validator.CollectionSize(x => x.LCD, 0, 500);
		foreach (var item in LGY) validator.Results.AddRange(item.Validate().Errors);
		foreach (var item in LPR) validator.Results.AddRange(item.Validate().Errors);
